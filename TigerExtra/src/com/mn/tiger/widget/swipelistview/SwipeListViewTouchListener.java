...
public class SwipeListViewTouchListener implements View.OnTouchListener
{
	...
	public SwipeListViewTouchListener(final SwipeListView swipeListView)
	{
		...
		swipeListView.setRecyclerListener(new RecyclerListener()
		{
			...
			{
				((SwipeListViewHolder)view.getTag()).getFrontView().animate().cancel();
	 ...
	 *            Front view
	 */
	private void setFrontView(final View frontView, final int childPosition)
	{
	...
	{
		SwipeListViewHolder holder = (SwipeListViewHolder) swipeListView.getChildAt(
				position - swipeListView.getFirstVisiblePosition()).getTag();
		final View child = holder.getFrontView();
	...
	protected void unselectedChoiceStates()
	{
		...
		for (int i = 0; i < checked.size(); i++)
		{
			...
			{
				reloadChoiceStateInView(((SwipeListViewHolder)swipeListView.
						getChildAt(i - start).getTag()).getFrontView(), i);
			}
			checked.set(i, false);
		}
		swipeListView.onChoiceEnded();
	...
	void closeOpenedItems()
	{
		if (opened != null)
		{
			...
			for (int i = start; i <= end; i++)
			{
				if (opened.get(i))
				{
					closeAnimate(((SwipeListViewHolder)swipeListView.
							getChildAt(i - start).getTag()).getFrontView(), i);
				}
			}
		}
		...
		switch (MotionEventCompat.getActionMasked(motionEvent))
		{
			case MotionEvent.ACTION_DOWN:
			{
				...
				for (int i = 0; i < childCount; i++)
				{
					...
					{
						setParentView(child);
						setFrontView(((SwipeListViewHolder)child.getTag()).getFrontView(), childPosition);
						...
						setBackView(((SwipeListViewHolder)child.getTag()).getBackView());
						break;
					}
				}
